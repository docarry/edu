...
function btnRun(){
    let newList = '';
    for(let i = 0; i < 3 && moreList <= listArray.length; i++){
        newList += `<li><a href="#"><img src="${listArray[moreList][0]}" alt="${listArray[moreList][1]}"><span>${listArray[moreList][2]}</span><span>${listArray[moreList][3]}</span></a></li>`;
        moreList++;
    }if(moreList >= listArray.length){
        moreBtn.style.display = 'none';
    }
    nikeList.insertAdjacentHTML('beforeend', newList);
}

moreBtn.addEventListener('click', btnRun);